...
function normalizeHotel(hotel) {
   ...
   * - price.current = current nightly price
   * - price.price_per_night = nightly price
   * - price.total / total_price = explicit total when available
   *
...
function enrich(hotel) {
  ...
  const nightly =
    hotel.nightly != null
      ? hotel.nightly
      : (
          hotel.suppliedTotal != null
            ? hotel.suppliedTotal / n
            : null
        );
...
async function searchLive() {
  ...
  try {
    ...
    lastSearchMeta = {
      ...
      sourceUrl:
        payload?.search_metadata?.source_url ??
        null
    };
...
function showHotel(hotel) {
  // All means all Google Hotels results.
  // Loyalty-program filters narrow the set when selected.
  return true;
}
...
function render() {
  ...
  $("resultMeta").innerHTML =
    ...
    `${$("guests").value} guests` +
    `${
      lastSearchMeta.sourceUrl
        ? ` · <a href="${escapeHtml(lastSearchMeta.sourceUrl)}" target="_blank" rel="noopener">Google Hotels source</a>`
        : " · Live price source: Google Hotels"
    }`;
  ...
  $("results").innerHTML = hotels.map(h => {
    ...
    const booking =
      `<button
        ...
        type="button"
        data-book-hotel="${escapeHtml(h.name)}"
        data-book-location="${escapeHtml($("city").value || "")}"
      >Book / compare</button>`;
      ...
      <article class="hotel">
        ...
        <div class="hotel-price">
          <div>
            ...
            <div class="price-source">Live price via Google Hotels</div>
            <div class="effective">€${Math.round(h.effective)}</div>
            <div class="nightly">€${Math.round(h.effectiveNightly)} / night</div>

            <div class="price-detail">
              Google Hotels nightly: €${Math.round(h.nightly)}
              <br>
              Google Hotels stay: €${Math.round(h.total)}
  ...
  fetch(
    ...
    .then(
      (data) => {
        ...
        if (!links.length) {
          container.innerHTML =
            `<p class="booking-empty">No booking platform links were returned.</p>`;
          return;
        }
    ...
    .catch(
      (error) => {
        ...
        if (container) {
          container.innerHTML =
            `<p class="booking-empty">${escapeHtml(error.message)}</p>`;
        }
      }
    );
}
...
function setup() {
  ...
  // Do not silently activate these filters on first load.
  if ($("onlyBenefits")) {
