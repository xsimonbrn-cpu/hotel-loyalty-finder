...
async function searchLive() {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Invalid response from hotel API (${response.status})`
      );
    }
...
function render() {
  ...
  if (
    ...
    if (emptyState) {
      ...
      if (text) {
        text.textContent =
          "No results yet. Search for a city.";
      }
    }
        ...
        const booking =
          `
            <button
              ...
              type="button"
              data-book-hotel="${escapeHtml(hotel.name)}"
              data-book-location="${escapeHtml(city)}"
            >
              Book / compare
            </button>
          `;
...
async function openBookingModal(
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Invalid booking-links response."
      );
    }
