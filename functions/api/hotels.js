...
function num(x){
 if(x==null||x==="")return null;if(typeof x==="number")return Number.isFinite(x)?x:null;
...
function url(x){
 ...
 if(x&&typeof x==="object")return url(x.original)||url(x.original_image)||url(x.image)||url(x.thumbnail)||url(x.url)||url(x.link)||url(x.website)||url(x.website_url);
 return null
}
function urls(p){let official=url(first(p,["official_website","official_website_url","hotel_website","website","website_url","official_url"]));let booking=url(first(p,["booking_url","hotel_url","link"]));for(const x of (Array.isArray(p?.prices)?p.prices:[])){const u=url(x);if(u&&!booking)booking=u}return {official_url:official,booking_url:booking}}
function amenity(v){
 const t=norm(v);
 if(/\b(pool|swimming pool|indoor pool|outdoor pool|infinity pool|rooftop pool|pool area)\b/.test(t))return "Pool";
 if(/\b(spa|wellness|wellness centre|wellness center|massage|thermal spa|health spa|wellness area|beauty spa)\b/.test(t))return "Spa";
 if(/\b(sauna|steam room|steam bath|steam sauna|hammam|hamam|infrared sauna|saunarium|finnish sauna)\b/.test(t))return "Sauna";
 if(/\b(fitness|gym|fitness centre|fitness center|workout|exercise room)\b/.test(t))return "Fitness";
 if(/\b(breakfast|buffet breakfast|breakfast included)\b/.test(t))return "Breakfast";
 if(/\b(parking|car park|garage|valet parking|private parking)\b/.test(t))return "Parking";
 return null
}
function amenities(p){
 const vals=[];
 const add=v=>{if(v==null)return;if(Array.isArray(v))v.forEach(add);else if(typeof v==="object"){add(v.name);add(v.label);add(v.title);add(v.text);add(v.description);add(v.type)}else vals.push(String(v))};
 ["amenities","facilities","amenity","hotel_amenities","amenities_text","facilities_text","services","highlights","property_highlights","description"].forEach(k=>add(p?.[k]));
 const result=new Set();for(const v of vals)for(const part of v.split(/[,;|•\n]/)){const a=amenity(part);if(a)result.add(a)}
 return [...result]
}
function images(p){
 const out=[];const add=x=>{const u=url(x);if(u&&!out.includes(u))out.push(u)};
 ["images","photos","photo_urls","gallery"].forEach(k=>{if(Array.isArray(p?.[k]))p[k].forEach(add)});
 ["original_image","image","image_url","thumbnail","thumbnail_url","photo","cover_image"].forEach(k=>add(p?.[k]));
 return out
}
function address(p){const a=p?.address||p?.location||p?.formatted_address;if(typeof a==="string")return clean(a);if(a&&typeof a==="object")return clean([a.street,a.street_address,a.housenumber,a.city,a.postal_code,a.zip,a.country].filter(Boolean).join(", "));return null}
function classify(p){
 const name=clean(p?.name||p?.hotel_name||p?.title),text=norm(`${name} ${p?.brand||""} ${p?.brand_name||""} ${p?.chain||""} ${p?.chain_name||""}`);
 for(const {program,brand} of [...flat].sort((a,b)=>b.brand.length-a.brand.length))if(text.includes(norm(brand)))return {program,brand,chain:program.split(" ")[0]};
 return {program:null,brand:p?.brand||p?.brand_name||null,chain:p?.chain||p?.chain_name||null}
}
function price(p,nights){
 let night=num(p?.rate_per_night),total=num(p?.total_rate);night=night??num(first(p,["extracted_price","price_per_night","nightly_price","price"]));total=total??num(first(p,["extracted_total","total_price","total"]));
 for(const x of (Array.isArray(p?.prices)?p.prices:[])){night=night??num(first(x,["rate_per_night","price_per_night","price"]));total=total??num(first(x,["total_rate","total_price","total"]));if(night!=null||total!=null)break}
...
export async function onRequest(context){
 ...
 const q=new URL(request.url),location=clean(q.searchParams.get("location")),ci=q.searchParams.get("check_in"),co=q.searchParams.get("check_out"),adults=Math.max(1,Math.min(20,Number(q.searchParams.get("adults")||2))),pages=Math.max(1,Math.min(10,Number(q.searchParams.get("pages")||10))),stream=q.searchParams.get("stream")==="1";
 if(!location)return json({error:"Missing location"},400);if(!/^\d{4}-\d{2}-\d{2}$/.test(ci)||!/^\d{4}-\d{2}-\d{2}$/.test(co)||co<=ci)return json({error:"Invalid dates"},400);if(!env?.SERPAPI_KEY)return json({error:"SERPAPI_KEY missing"},500);
 const base={q:location,check_in_date:ci,check_out_date:co,adults,currency:"EUR",gl:"de",hl:"en"};
 const requestedStars=Number(q.searchParams.get("min_stars")||0),requestedAmenity=clean(q.searchParams.get("amenity")),requestedProgram=clean(q.searchParams.get("program"));
 const queries=[base];
 if(requestedStars)queries.push({...base,hotel_class:`${requestedStars},5`});
 if(requestedAmenity)queries.push({...base,q:`${location} hotels with ${requestedAmenity.toLowerCase()}`});
