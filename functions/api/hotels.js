...
export async function onRequest(context){
 ...
 const q = new URL(request.url);
  const location = clean(q.searchParams.get("location"));
  const ci = q.searchParams.get("check_in");
  const co = q.searchParams.get("check_out");
  const adults = Math.max(1, Math.min(20, Number(q.searchParams.get("adults") || 2)));
  const pages = Math.max(1, Math.min(10, Number(q.searchParams.get("pages") || 10)));
